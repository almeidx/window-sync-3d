...
if (new URLSearchParams(window.location.search).get("clear")) {
  ...
  function updateNumberOfCubes() {
    ...
    for (let i = 0; i < wins.length; i++) {
      ...
      let c;
      if (i == 0) {
        c = new THREE.Color("hsl(230, 80%, 75%)");
      } else if (i == 1) {
  ...
  function render() {
    ...
    for (let i = 0; i < cubes.length; i++) {
      ...
      complexSphere.position.x =
        complexSphere.position.x +
        (posTarget.x - complexSphere.position.x) * falloff;
      complexSphere.position.y =
        complexSphere.position.y +
        (posTarget.y - complexSphere.position.y) * falloff;

      complexSphere.rotation.x = _t * 0.5;
      complexSphere.rotation.y = _t * 0.3;
  ...
  function updateComplexSphere(complexSphere, elapsedTime) {
    ...
    sphereWireframeInner.material.opacity = Math.abs(
      Math.cos((elapsedTime + 0.5) / 0.9) * 0.5
    );
    sphereWireframeOuter.material.opacity = Math.abs(
      Math.cos(elapsedTime / 0.9) * 0.5
    );
  }
